            ...
            if (closestDistance < entity.getSightRadius()){
                ...
                if (isRanged) {
                    ...
                    if (closestDistance < attackRange) {
                        // Au contact : on s'arrête (pour ne pas traverser l'ennemi)
                        entity.chooseDirection(nullptr);
                    } else {
                        // On charge
                        entity.chooseDirection(target);
                    ...
                    if (lastShotTime.find(entity.getName()) == lastShotTime.end() ||
                        ...
                        if (entity.consumeStamina(entity.getStaminaAttackCost())) {
                            if (isRanged) {
                                ...
                                // Dégâts directs (Mêlée)
                                closestEnemy->takeDamage(entity.getDamage());
                            }
    ...
    for (auto &entity : entities) {
        for (auto &other : entities) {
            if (&entity != &other && other.getIsAlive()) {
                ...
                int minDist = entity.getRad() + other.getRad();
                if (dist < minDist) {
                    float overlap = 0.5f * (minDist - dist);
                    float nx = (dist > 0) ? dx/dist : 1;
                    float ny = (dist > 0) ? dy/dist : 0;
                    entity.setX(entity.getX() + (int)(nx * overlap));
                    entity.setY(entity.getY() + (int)(ny * overlap));
                }
            }
        }
    }
...
void Simulation::updateProjectiles() {
    projectiles.erase(std::remove_if(projectiles.begin(), projectiles.end(), [&](Projectile& proj) {
        ...
        for (auto &entity : entities) {
            if (entity.getIsAlive() && entity.getName() != proj.getShooterName()) {
                int dx = proj.getX() - entity.getX();
                int dy = proj.getY() - entity.getY();
                if (std::sqrt((float)dx*dx + dy*dy) < proj.getRadius() + entity.getRad()) {
...
void Simulation::cleanupDead() {
    entities.erase(std::remove_if(entities.begin(), entities.end(), [](const Entity &e){ return !e.getIsAlive(); }), entities.end());
}
...
void Simulation::render(SDL_Renderer *renderer, bool showDebug) {
    ...
    if (panelCurrentX < WINDOW_WIDTH) drawStatsPanel(renderer, (int)panelCurrentX);
}


void Simulation::drawStatsPanel(SDL_Renderer* renderer, int panelX) {
    auto float_to_string = [](float val, int precision = 2) {
        std::stringstream ss; ss << std::fixed << std::setprecision(precision) << val; return ss.str();
    };
    ...
    stringRGBA(renderer, x, y, ("Dmg: " + std::to_string(entity.getDamage())).c_str(), statColor.r, statColor.g, statColor.b, 255); y += lineHeight;
}