...
void Entity::calculateDerivedStats() {
    ...
    // Application finale PV Max du trait
...
void Entity::draw(SDL_Renderer* renderer, const Camera& cam, bool showDebug) {
    ...
    // On calcule la position et la taille à l'écran en fonction du zoom et du déplacement
    int screenX = (int)((x - cam.x) * cam.zoom);
    int screenY = (int)((y - cam.y) * cam.zoom);
    int screenRad = (int)(rad * cam.zoom);

    // Optimisation : Si hors écran, on ne dessine pas
    ...
    if (barWidth < 2) barWidth = 2; // Largeur minimale
...
void Entity::update(int speedMultiplier) {
    ...
    // --- 7. COLLISIONS (Avec le MONDE, pas la fenêtre) ---
    bool collided = false;
    // On utilise WORLD_WIDTH / WORLD_HEIGHT ici
    if (x < rad) { x = rad; collided = true; }
    else if (x > WORLD_WIDTH - rad) { x = WORLD_WIDTH - rad; collided = true; }

    if (y < rad) { y = rad; collided = true; }
    else if (y > WORLD_HEIGHT - rad) { y = WORLD_HEIGHT - rad; collided = true; }
...
void Entity::chooseDirection(int target[2]) {
    if(target != nullptr){
        ...
        // La cible peut être temporairement hors map, la collision corrigera
...
void Entity::knockBackFrom(int sourceX, int sourceY, int force) {
    ...
    // Clamp avec WORLD_WIDTH / WORLD_HEIGHT
